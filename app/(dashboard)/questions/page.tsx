...
import { questionColumns } from "@/app/(dashboard)/questions/columns";
...
const QuestionsPage = () => {
    ...
    useEffect(() => {
        ...
        startTransition(async () => {
            const result = await questionService.getList(pagination.pageNumber, pagination.pageSize, sorting.orderBy, sorting.sortBy, debouncedSearchQuery, debouncedWithDeleted);
            // console.log(result);
    ...
    return (
        <Card className=''>
            <CardHeader>
                <CardTitle>
                    <div className='rounded-md flex items-center justify-between'>
                        <h1 className='font-semibold'>All Question</h1>

                        <Link href={'./questions/form'}>
            ...
            <CardContent>
                <DataTable
                    columns={questionColumns(refreshData)}
                    data={data}
                    loading={isPending}
                    pagination={{
