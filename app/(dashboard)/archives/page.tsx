...
import { archiveColumns } from "@/app/(dashboard)/archives/columns";
...
const TopicsPage = () => {
    ...
    const debouncedLessonId = useDebounce(year, 500);


    useEffect(() => {
        startTransition(async () => {
            const result = await archiveService.getList(pagination.pageNumber, pagination.pageSize, sorting.orderBy, sorting.sortBy, debouncedSearchQuery, debouncedWithDeleted, debouncedSubjectId, debouncedLessonId);
            // console.log(result);
    ...
    return (
        <Card>
            ...
            <CardContent>
                <DataTable
                    columns={archiveColumns(refreshData)}
                    data={data}
                    loading={isPending}
                    pagination={{
