...
import { topicContentColumns } from "@/app/(dashboard)/topic-content/columns";
import { topicContentService } from "@/services/topic-content.service";


const TopicsContentPage = () => {
    ...
    useEffect(() => {
        ...
        startTransition(async () => {
            const result = await topicContentService.getList(pagination.pageNumber, pagination.pageSize, sorting.orderBy, sorting.sortBy, debouncedSearchQuery, debouncedWithDeleted, debouncedTopicId);
            // console.log(result);
    ...
    }, [pagination.pageNumber, pagination.pageSize, sorting.sortBy, sorting.orderBy, debouncedSearchQuery, refresh, debouncedWithDeleted, debouncedTopicId]);
    ...
    return (
        <Card>
                ...
                <div className='mb-4 flex items-center gap-2'>
                    ...
                    <div>
                        <SelectList
                            ...
                            value={topicId}
                            onValueChange={(val) => {
                                setTopicId(Number(val));
                            }}
                            loadItems={loadTopics}
                            placeholder="Select Topic"
                            emptyText={subjectId ? "No topic found." : "Select topic first"}
                            className="w-full"
                        />
                    </div>
            ...
            <CardContent>
                <DataTable
                    columns={topicContentColumns(refreshData)}
                    data={data}
                    loading={isPending}
                    pagination={{
...
export default TopicsContentPage;
